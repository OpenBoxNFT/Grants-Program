...
| 2. |Use existing application chains and pallets developed based on Substrate to directly realize cross-chain NFT transactions | Developing a new application chain to achieve cross-chain interoperability support involves need a long cycle, and there is no plan to issue tokens at this stage. The other uses the existing MoonBeam developed based on Substrate and the existing pallets and interfaces on Unique Network to achieve interoperability. Unique Network does not seem to be doing enough on Ethereum to support cross-chain interoperability. It is impossible to realize cross-chain contract purchases smoothly directly and may need to rely on centralized steps or centralized trading pools to achieve step by step. The specific implementation may need to be put at a later stage|  
...
-![image](https://user-images.githubusercontent.com/103482054/169665995-71ef1572-a9e0-4d8d-8354-8883fc6dc5e3.png)
